...
class EditorTestCase(TestCase):
    ...
    def test_submit_settings_description(self):
        c = Client()
        c.login(username="test", password="test")
        response = c.post("/editor/settings/1/", {"description": "Descripción de un mapaquiz"})
        assert response.status_code == 302
        response = c.get("/editor/provincias/")
    ...
    def test_submit_settings_both(self, MockContainerClient):
        ...
        c = Client()
        c.login(username="test", password="test")
        response = c.post("/editor/settings/1/", {"front_image": open("mapaly.png", "rb"), "description": "Descripción de un mapaquiz"})
        assert response.status_code == 302
        response = c.get("/editor/provincias/")
        assert response.status_code == 200
        self.assertContains(response, "mapaly.png")
